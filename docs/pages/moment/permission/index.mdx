...
```http
POST https://inprocess.fun/api/moment/permission
```
...
```bash [cURL]
curl -X POST https://inprocess.fun/api/moment/permission \
...
const payload: AddPermissionRequest = {
  moment: {
    collectionAddress: "0x297F21AAc502571db2c7dBFD271E68Dae7F2cE1E",
    tokenId: "1",
  },
...
This endpoint grants admin permissions (permission bit 2) to the specified address for the given Moment. The permission is added on-chain via the Zora Creator 1155 contract's `addPermission` function.

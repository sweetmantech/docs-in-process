...
```http
GET https://inprocess.world/api/moment
```
...
curl "https://inprocess.world/api/moment?collectionAddress=0x06a701Ae65582B92Af48cDff45a8B20DcA3714cA&tokenId=1"
```
...
const response = await fetch(`https://inprocess.world/api/moment?${params}`);
...
const response = await fetch(
  `https://inprocess.world/api/moment?${queryString}`
);
const data = await response.json();
console.log(data);
```

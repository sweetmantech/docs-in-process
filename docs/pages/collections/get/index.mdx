...
```http
GET https://inprocess.fun/api/collection
```
...
curl "https://inprocess.fun/api/collection?collectionAddress=0x06a701Ae65582B92Af48cDff45a8B20DcA3714cA&chainId=8453"

# Without chainId (uses default: 8453)
curl "https://inprocess.fun/api/collection?collectionAddress=0x06a701Ae65582B92Af48cDff45a8B20DcA3714cA"
```

```python [Python]
import requests

url = "https://inprocess.fun/api/collection"
params = {
...
const response = await fetch(`https://inprocess.fun/api/collection?${params}`);
...
const response = await fetch(
  `https://inprocess.fun/api/collection?${queryString}`
);
const data = await response.json();
console.log(data);
```
...
| chainId                | number | Chain ID where the collection exists          |

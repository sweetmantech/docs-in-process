...
```http
GET https://inprocess.fun/api/legacy/timeline
```
...
```bash [cURL]
curl -X GET "https://inprocess.fun/api/legacy/timeline?artist=0x29b6674c1562e31EdFd9709D9576e8E5F1e68d01&chainId=8453&limit=2&page=1&latest=true&hidden=true"
```
...
def get_timeline(page: int = 1, limit: int = 100, latest: bool = True, artist: str = None, chainId: int = 8453, hidden: bool = False):
    url = "https://inprocess.fun/api/legacy/timeline"
...
async function getTimeline(
  ...
  const response = await fetch(`https://inprocess.fun/api/legacy/timeline?${params}`);

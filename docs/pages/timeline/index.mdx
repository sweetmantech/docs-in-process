...
- **When `artist` is omitted**: Returns moments from the in•process collective timeline (moments that are being created, indexed).
...
### in•process Timeline (without artist)
...
def get_timeline(artist: str | None = None, page: int = 1, limit: int = 100, chain_id: int = 8453, hidden: bool = False, type: str | None = None):
    url = "https://inprocess.fun/api/timeline"
    params = {"page": page, "limit": limit, "chain_id": chain_id}
    if artist:

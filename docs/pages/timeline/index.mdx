...
```bash [cURL]
curl -X GET "https://inprocess.fun/api/timeline?limit=2&page=1&latest=true"
```

```python [Python]
import requests

def get_timeline(page: int = 1, limit: int = 20, latest: bool = True):
...
async function getTimeline(page = 1, limit = 20, latest = true) {
...
async function getTimeline(
  ...
  const response = await fetch(
    `https://inprocess.fun/api/timeline?page=${page}&limit=${limit}&latest=${latest}`
  );
  if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
  return await response.json();
}
